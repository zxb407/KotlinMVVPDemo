...
public class PullToRefreshView extends ViewGroup {
    ...
    private int topAndBottomOffset = 0;
    ...
    protected void onFinishInflate() {
        ...
        if (contentView instanceof NestedScrollView){
            ...
            for (int i = 0; i < ((ViewGroup)contentView).getChildCount(); i++) {
                if (((ViewGroup)contentView).getChildAt(i) instanceof NestedScrollView) {
                    scrollView = ((ViewGroup)contentView).getChildAt(i);
                    break;
                }
            }
        }
    ...
    private boolean isNeedIntercept() {
        if (deltaY > 20 && Math.abs(deltaX) < Math.abs(deltaY) && isContentScrollToTop() || getScrollY() < 0 - 10) {
            mStatus = PULL_DOWN;
            return true;
        }

        if (deltaY < -20 && Math.abs(deltaX) < Math.abs(deltaY) && isContentScrollToBottom() || getScrollY() > 0 + 10) {
            mStatus = PULL_UP;
            return true;
        }
    ...
    private boolean isContentScrollToTop() {
        return !ViewCompat.canScrollVertically(scrollView, -1) && topAndBottomOffset == 0;
    }
    ...
    public void setHeaderColorMode(int color) {
        mHeaderIndicator.setIndicatorMode(color);
    }

    public void setFooterColorMode(int color) {
        mFooterIndicator.setIndicatorMode(color);
    }
}
